...
describe('polygon', () => {
  ...
  it('finds closest point and signed distance on a triangle', () => {
    const tri = [vec2(0, 0), vec2(2, 0), vec2(0, 2)]
    const below = closestPointOnPolygon(vec2(1, -1), tri)!
    expect(below.signedDistance).toBeCloseTo(1, 6)
    expect(below.point.x).toBeCloseTo(1, 6)
    expect(below.point.y).toBeCloseTo(0, 6)
    expect(below.ny).toBeLessThan(0)

    const inside = closestPointOnPolygon(vec2(0.4, 0.4), tri)!
    expect(inside.signedDistance).toBeLessThan(0)
    expect(Math.abs(inside.signedDistance)).toBeCloseTo(0.4, 6)

    const outside = closestPointOnPolygon(vec2(-1, 1), tri)!
    expect(outside.signedDistance).toBeGreaterThan(0)
    expect(outside.point.x).toBeCloseTo(0, 6)
  })
  ...
  it('decomposes an L-shape into convex parts covering the original area', () => {
    const L = [vec2(0, 0), vec2(2, 0), vec2(2, 1), vec2(1, 1), vec2(1, 2), vec2(0, 2)]
...
describe('aabbFromShape', () => {
  ...
  it('computes bounding box for capsule, polyline and segment', async () => {
    const { aabbFromShape } = await import('./aabb.ts')
    const capBox = aabbFromShape(
      { kind: 'capsule', halfHeight: 2, radius: 0.5 },
      { x: 0, y: 0, angle: 0 },
    )
    ...
    const segBox = aabbFromShape(
      { kind: 'segment', a: { x: -1, y: 0 }, b: { x: 3, y: 4 } },
      { x: 0, y: 0, angle: 0 },
    )
